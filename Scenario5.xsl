        ...
        <recipeboxes>
            <table border="1">
                ...
                <xsl:for-each select="//recipes/recipe[.//comment]">
                    ...
                    <xsl:sort select="sum(//user_feedback/comment[recipe/@idref = current()/@id]/rating) div count(//user_feedback/comment[recipe/@idref = current()/@id])" data-type="number" order="descending"/>
